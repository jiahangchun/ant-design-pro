...
class BasicProfile extends Component {
  ...
  queryRealResult = data => {
    ...
    dispatch({
      ...
      const resultObj =
        typeof mockRequestResult == 'string' ? JSON.parse(mockRequestResult) : mockRequestResult;
      this.setState({
        resultObj: resultObj,
      });
    });
  ...
  changeDefaultParamValue = e => {
    ...
    let toUpdateData = Object.assign({}, this.state.data, { requestParamVos: paramData });
    this.setState({
      data: toUpdateData,
    });
  };
  ...
  render() {
    ...
    const requestColumn = [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <Input name={record.name} onChange={this.changeDefaultParamValue} defaultValue={text} />
          );
        },
      },
    ];
      ...
      <PageHeaderWrapper title="接口详情页" loading={loading}>
        <Card bordered={false}>
          <DescriptionList size="large" title="简要描述" style={{ marginBottom: 32 }}>
            <Description term="请求URL">
              <Popover content="点击生成Json请求数据" trigger="hover">
                <a onClick={() => this.queryRealResult(this.state.data)} type="link">
                  {this.state.data.url}
                </a>
              </Popover>
            </Description>
          ...
          <Result
            ...
            extra={
              <Button type="primary" onClick={() => this.queryRealResult(this.state.data)}>
                Try
              </Button>
            }
          >
